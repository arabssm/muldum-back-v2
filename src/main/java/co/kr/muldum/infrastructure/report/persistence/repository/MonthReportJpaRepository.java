...
public interface MonthReportJpaRepository extends JpaRepository<MonthReportEntity, Long> {
    ...
    @Query(value = "SELECT * FROM month_report m WHERE EXTRACT(MONTH FROM m.created_at) = :month", nativeQuery = true)
    List<MonthReportEntity> findByMonth(@Param("month") int month);
    ...
    Optional<MonthReportEntity> findByTeamIdAndStatus(Long teamId, ReportStatus status);
}
